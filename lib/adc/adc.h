...
// An ADC read that puts the chip in Idle mode during the read
uint16_t adc_quiet_read16(
    uint8_t adc_channel,
    uint8_t reference,
    uint8_t prescaler);

// This version compares the internal 1.1V reference with
// VCC.  For example, if VCC is 5V, then we would read
// int(1023 * 1.1 / 5) -> 225.  It's a way to save on
// a few external components.
uint16_t adc_quiet_read16_internal_ref(uint8_t prescaler);
